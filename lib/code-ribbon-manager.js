...
const NewItem = require('./new-item')
...
class CodeRibbonManager {
  ...
  constructor(state, viewRegistry) {
    ...
    this.subscriptions.add(atom.commands.add('.tree-view .full-menu [is*="tree-view-"]', {
      'code-ribbon:network-graph': (dispatch_event) => {
        ...
        try {
          ...
          let position = 0;
          this.addPatchWithNetworkGraph(this.cr_primary_container.getRoot(), position, repository);
        } catch (err) {
          crlogger.error(err);
        }
      }
    ...
    this.subscriptions.add(atom.commands.add('atom-pane.cr-patch', {
      'code-ribbon:network-graph': (dispatch_event) => {
        ...
        try {
          ...
          let position = crrc.children.indexOf(dispatch_event.currentTarget.model.parent) + 1;
          this.addPatchWithNetworkGraph(crrc, position, repository);
        } catch (err) {
          crlogger.error(err);
        }
      }
    ...
    this.subscriptions.add(atom.commands.add('atom-pane', {
      'code-ribbon:new-item': (dispatch_event) => {
        crdebug(dispatch_event);
        let crrc = this.cr_primary_container.getRoot()
        try {
          dispatch_event.stopImmediatePropagation();
  ...
  addPatchWithNetworkGraph(crrc, position, repository) {
    ...
    scrollPatchIntoView(focus_target, () => {
      crrc.cr_add_strip(position, [new CodeRibbonPatch({
        ...
        applicationDelegate: atom.applicationDelegate,
        config: atom.config,
        item: new NetworkGraph({repository})
      }),]);
      global_cr_update(); // this will cause the transition
    });
    metrics.event({
      name: 'Add graph network',
      type: 'interaction',
      location: position
    });
  }
